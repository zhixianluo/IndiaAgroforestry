...
gen per_hect_tree = per_acre_tree*2.47
...
by ECO_NAME, sort : su tree_grow_dummy tot_tree per_hect_tree ratio_hort_tot_tree ratio_timb_tot_tree ratio_bmbo_tot_tree
...
gen tot_land_operate_hect = tot_land_operate_acre/2.47
gen tot_land_operate_acre_2 = own_land_acre + coowned_land_acre
gen tot_land_operate_hect_2 = tot_land_operate_acre_2/2.47
...
ttest per_hect_tree, by(landhold)
...
by heard_SMAFF2, sort : su tot_tree per_hect_tree ratio_timb_tot_tree ratio_bmbo_tot_tree
by partcp_SMAFF2, sort : su tot_tree per_hect_tree ratio_timb_tot_tree ratio_bmbo_tot_tree

gen ag_supp=2 if partcp_SMAFF2==1
replace ag_supp=1 if partcp_SMAFF2==0
replace ag_supp=0 if ag_supp==.

* 3.1.1 Motivation for program participation and non-participation 

* Data for Table SI 3
tab why_parti_agroforest

* Data for Table SI 4
tab why_notparti_agroforest

* 3.2 Test differences in means

ttest tot_tree if ag_supp<2, by(ag_supp)
ttest per_hect_tree if ag_supp<2, by(ag_supp)
...
ttest per_hect_tree if ag_supp==0 | ag_supp==2, by(ag_supp)
...
by ag_supp, sort : summarize tot_tree, detail
by ag_supp, sort : summarize per_hect_tree, detail

by ag_supp, sort : centile tot_tree 
centile per_hect_tree

median tot_tree if ag_supp<2, by(ag_supp)
median per_hect_tree if ag_supp<2, by(ag_supp)

median tot_tree if ag_supp==0 | ag_supp==2, by(ag_supp)
median per_hect_tree if ag_supp==0 | ag_supp==2, by(ag_supp)


* 4 Data for Figure SI 2, Village exposure to agroforestry programs
...
* Counting if the last planting year before 20000 
count if treeplnt_year_1<2000

* 6 Data for Table SI 7

by ag_supp, sort : summarize agroeco_dummy_1 agroeco_dummy_2 agroeco_dummy_3 ///
dist_Harda dist_Hoshangabad dist_Jabalpur dist_Sehore dist_Shajapur ///
tmin_rabi_1995_2014 tmax_rabi_1995_2014 prec_rabi_1995_2014 tmin_kharif_1995_2014 ///
tmax_kharif_1995_2014 prec_kharif_1995_2014 gps_resp_housealtitude yr2000_tree_cover_percent ///
dist_rangeoffice kvk_distance Distancetotheclosestgovernme Distancetotheclosestprivate /// 
puccaroad_distance inputmarket_distance Distancetotheclosestforestry cermarket_distance ///
pulsmarket_distance fruitmarket_distance vegmarket_distance   /// 
caste_ST caste_sc caste_obc caste_gen hh_mem_total  resp_age resp_schoolyr  ///
tot_land_operate_hect  tot_land_operate_hect_2 credit_aces  dummy_irrigation_GW



tab ag_supp
by ag_supp, sort : tab landhold

by ag_supp, sort: summarize caste_gen
by ag_supp, sort: summarize caste_obc
by ag_supp, sort: summarize caste_sc
by ag_supp, sort: summarize caste_ST



by ag_supp, sort: summarize agroeco_dummy_1
by ag_supp, sort: summarize agroeco_dummy_2
by ag_supp, sort: summarize agroeco_dummy_3
...
*7 Create datasets for CovSel algorithm
use TemporaryDataset
keep qre_uniqueid resp_age resp_schoolyr hh_mem_total tot_land_operate_acre ///
puccaroad_distance inputmarket_distance cermarket_distance vegmarket_distance ///
fruitmarket_distance pulsmarket_distance kvk_distance Distancetotheclosestgovernme ///
Distancetotheclosestprivate Distancetotheclosestforestry dist_rangeoffice ///
gps_resp_housealtitude tmin_rabi_1995_2014 tmax_rabi_1995_2014 ///
prec_rabi_1995_2014 tmin_kharif_1995_2014 tmax_kharif_1995_2014 prec_kharif_1995_2014 ///
yr2000_tree_cover_percent tree_grow_dummy tot_tree per_hect_tree ratio_hort_tot_tree ///
ratio_timb_tot_tree ratio_bmbo_tot_tree dist_Hoshangabad dist_Jabalpur dist_Sehore ///
dist_Shajapur agroeco_dummy_1 agroeco_dummy_2 caste_obc caste_sc caste_ST ///
credit_aces tot_land_operate_acre_2 dummy_irrigation_GW heard_SMAFF2 partcp_SMAFF2

save Varsfor_COVSEL_Final, replace
clear all

* 8 Matching models: IPWRA

use TemporaryDataset

* 8.1 Outcome: tree_grow_dummy

* Determine min and max values of matching covariates by treatment group

global control1 agroeco_dummy_1 caste_obc cermarket_distance dist_Hoshangabad /// 
dist_rangeoffice Distancetotheclosestgovernme dummy_irrigation_GW hh_mem_total /// 
inputmarket_distance kvk_distance pulsmarket_distance resp_age resp_schoolyr /// 
tot_land_operate_acre tot_land_operate_acre_2 


tabstat $control1 

global cont1 cermarket_distance Distancetotheclosestgovernme hh_mem_total /// 
inputmarket_distance kvk_distance pulsmarket_distance resp_age resp_schoolyr ///
tot_land_operate_acre tot_land_operate_acre_2 
...
keep if Support==1  // Drop observations without common support (i.e., beyond 5% of min and max values )
drop Support
...
di invFtail(139,139,0.975)
di invFtail(139,139,0.025)

di invFtail(83,83,0.975)
di invFtail(83,83,0.025)  

* Variance ratio for pulsmarket_distance is outside the guideline for ag_supp==2, but retain this covariate because it is within both balance guidelines for ag_supp==1 and SMD

* Estimate ATET

teffects ipwra (tree_grow_dummy $control1, logit) (ag_supp $control1), atet aeq
teffects ipwra (tree_grow_dummy $control1, logit) (ag_supp $control1), atet aeq level(90)

bysort ag_supp: su tree_grow_dummy

clear all

* 8.2 Outcome: tot_tree 

use TemporaryDataset

* Determine min and max values of matching covariates by treatment group

global control2 caste_obc dist_Jabalpur Distancetotheclosestprivate /// 
gps_resp_housealtitude hh_mem_total inputmarket_distance resp_age ///
tmax_rabi_1995_2014 tmin_kharif_1995_2014 tmin_rabi_1995_2014 ///
tot_land_operate_acre tot_land_operate_acre_2 vegmarket_distance /// 
yr2000_tree_cover_percent 


tabstat $control2 

global cont2 dist_Jabalpur Distancetotheclosestprivate /// 
gps_resp_housealtitude hh_mem_total inputmarket_distance resp_age ///
tmax_rabi_1995_2014 tmin_kharif_1995_2014 tmin_rabi_1995_2014 ///
tot_land_operate_acre tot_land_operate_acre_2 vegmarket_distance /// 
yr2000_tree_cover_percent  
...
su tot_tree
replace tot_tree = tot_tree / 1000 // Convergence not attained, so tot_tree is replaced
...
di invFtail(84,84,0.975)
di invFtail(84,84,0.025)
...
teffects ipwra (tot_tree $control2) (ag_supp $control2), atet aeq level(90)
bysort ag_supp: su tot_tree

clear all

* 8.3 Outcome: per_hect_tree

use TemporaryDataset

* Determine min and max values of matching covariates by treatment group

global control3 Distancetotheclosestforestry Distancetotheclosestgovernme ///
Distancetotheclosestprivate dummy_irrigation_GW fruitmarket_distance ///  
prec_kharif_1995_2014 pulsmarket_distance resp_schoolyr tmin_rabi_1995_2014 /// 
tot_land_operate_acre tot_land_operate_acre_2 yr2000_tree_cover_percent 



tabstat $control3 

global cont3 Distancetotheclosestforestry Distancetotheclosestgovernme ///
Distancetotheclosestprivate fruitmarket_distance ///  
prec_kharif_1995_2014 pulsmarket_distance resp_schoolyr tmin_rabi_1995_2014 /// 
tot_land_operate_acre tot_land_operate_acre_2 yr2000_tree_cover_percent   
...
quietly teffects ipwra (per_hect_tree $control3) (ag_supp $control3), atet aeq 
...
* For reasons analogous to those given in section 5.1, retain all covariates.

* Estimate ATET

teffects ipwra (per_hect_tree $control3) (ag_supp $control3), atet aeq
teffects ipwra (per_hect_tree $control3) (ag_supp $control3), atet aeq level(90)

bysort ag_supp: su per_hect_tree

clear all

* 8.4 Outcome: ratio_hort_tot_tree
...
global control4 agroeco_dummy_1 caste_obc caste_sc caste_ST credit_aces /// 
dist_rangeoffice Distancetotheclosestprivate dummy_irrigation_GW /// 
gps_resp_housealtitude hh_mem_total prec_kharif_1995_2014 pulsmarket_distance /// 
resp_age resp_schoolyr tmin_kharif_1995_2014 tmin_rabi_1995_2014 /// 
tot_land_operate_acre_2 yr2000_tree_cover_percent 

   
tabstat $control4  

global cont4 dist_rangeoffice Distancetotheclosestprivate /// 
gps_resp_housealtitude hh_mem_total prec_kharif_1995_2014 pulsmarket_distance /// 
resp_age resp_schoolyr tmin_kharif_1995_2014 tmin_rabi_1995_2014 /// 
tot_land_operate_acre_2 yr2000_tree_cover_percent 
...
di invFtail(69,69,0.975)
di invFtail(69,69,0.025)

** Variance ratio for credit_aces is outside the guideline for ag_supp==2, but retain this covariate because it is within both balance guidelines for ag_supp==1 and SMD 

* Estimate ATET

teffects ipwra (ratio_hort_tot_tree $control4) (ag_supp $control4), atet aeq
teffects ipwra (ratio_hort_tot_tree $control4) (ag_supp $control4), atet aeq level(90)

bysort ag_supp: su ratio_hort_tot_tree

clear all

* 8.5 Outcome: ratio_timb_tot_tree 
...
global control5 agroeco_dummy_2 caste_sc Distancetotheclosestgovernme /// 
dummy_irrigation_GW gps_resp_housealtitude kvk_distance prec_kharif_1995_2014 /// 
resp_age resp_schoolyr tmin_rabi_1995_2014 tot_land_operate_acre vegmarket_distance 


tabstat $control5 

global cont5 Distancetotheclosestgovernme gps_resp_housealtitude ///
kvk_distance prec_kharif_1995_2014 resp_age resp_schoolyr ///
tmin_rabi_1995_2014 tot_land_operate_acre vegmarket_distance 
...
teffects ipwra (ratio_timb_tot_tree $control5) (ag_supp $control5), atet aeq level(90)
bysort ag_supp: su ratio_timb_tot_tree

clear all

* 8.6 Outcome: ratio_bmbo_tot_tree
...
global control6 agroeco_dummy_2 caste_obc caste_ST dist_Jabalpur dist_rangeoffice /// 
Distancetotheclosestforestry dummy_irrigation_GW fruitmarket_distance hh_mem_total /// 
kvk_distance resp_age resp_schoolyr tmin_kharif_1995_2014 tot_land_operate_acre /// 
tot_land_operate_acre_2 yr2000_tree_cover_percent


tabstat $control6  

global cont6 dist_rangeoffice Distancetotheclosestforestry ///
fruitmarket_distance hh_mem_total kvk_distance resp_age resp_schoolyr ///
tmin_kharif_1995_2014 tot_land_operate_acre /// 
tot_land_operate_acre_2 yr2000_tree_cover_percent  
...
quietly teffects ipwra (ratio_bmbo_tot_tree $control6) (ag_supp $control6), atet aeq 
...
di invFtail(103,103,0.975)
di invFtail(103,103,0.025)
...
teffects ipwra (ratio_bmbo_tot_tree $control6) (ag_supp $control6), atet aeq level(90)
...
* 9 Prediction models: lasso

use TemporaryDataset

* 9.1 Define groups of potential predictors
...
global hhcharec caste_gen caste_obc caste_sc caste_ST hh_mem_total resp_age resp_schoolyr tot_land_operate_acre tot_land_operate_acre_2 credit_aces dummy_irrigation_GW yr2000_tree_cover_percent

* 9.2 Define sample (untreated farmers only)

drop if ag_supp==1 | ag_supp==2

* 9.3 Lasso models by outcome variable 
...
lasso linear tot_tree $govtfclt $hhcharec $distdummy $mktcharec $agroecocharec, cluster (village) rseed(161176)
lassogof, postselection 
estimate store M1ap2 
lassocoef M1ap2, display(coef, standardized)

lasso linear per_hect_tree $govtfclt $hhcharec $distdummy $mktcharec $agroecocharec, cluster (village) rseed(171176)
lassogof, postselection 
...
lasso linear ratio_bmbo_tot_tree $govtfclt $hhcharec $distdummy $mktcharec $agroecocharec, cluster (village) rseed(171176)
lassogof, postselection
...
* 10 Create dataset for Figure SI 4, Planting years for commercial tree species planted by farmers
...
save Tree_Plant_Year, replace
clear all


*11 Primary benefit from main tree species
use TemporaryDataset.dta
tab sale_impact, missing
tab incdiversif_impact, missing
tab cropprod_impact, missing

drop if sale_impact==.

su sale_impact incdiversif_impact cropprod_impact

count if sale_impact==1 | incdiversif_impact==1
scalar income = int(100*147/239)
scalar list income

count if cropprod_impact==1
scalar consumption = int(100*111/239)
scalar list consumption

count if cropprod_impact==1 & (sale_impact==1 | incdiversif_impact==1)
scalar consumption_only = int(100-100*95/111)
scalar list consumption_only
