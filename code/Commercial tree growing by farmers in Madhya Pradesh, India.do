...
use "Commercial tree growing_HH_Vill_GIS.dta"
...
save TNC_AGF_MP, replace
clear all


* 2 Figure 1. Tree growing outcomes by landholding size: raw data
...
* 3 Figure 2. Tree growing outcomes by exposure to agroforestry programs: raw data
...
* 4 Data for Supplementary Fig 1
by village, sort : tabulate heard_SMAFF2, sort

* 5 Data for Supplementary Fig 3. Planting dates for commercial tree species planted by farmers
...
* Changed from probit to logit

* Check puccaroad_distance
bysort tree_grow_dummy ag_supp: count if puccaroad_distance==0
bysort tree_grow_dummy ag_supp: count if puccaroad_distance!=0
* JRV: Has too few nonzero values, so drop from this model and subsequent ones
...
* JRV: Retain all covariates because all satisfy both balance criteria for ag_supp==1, nearly all satisfy both for ag_supp==2, and all satisfy at least one criterion for ag_supp==2.
...
tot_land_operate_acre_2
* JRV: Have dropped puccaroad_distance
...
* JRV: For reasons analogous to those given above, retain all covariates.
...
use TemporaryDataset
* JRV: Restrict sample to farmers who grow trees
...
note: x's show which controls were selected by lasso
...
* JRV: Estimate remaining models on sample that includes only farmers who grow trees
keep if tree_grow_dummy==1
