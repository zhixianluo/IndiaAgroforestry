...
## 1st small,  then large without adding alphabet
## in df1, we use different scale for Y axia (tot tree, tree density)
...
## Req: Order is maintained as it is 
...
## Note: We use mefian instead of mean for vars:Total tree, Tree dens/acre in df3
## To estimaate SE from median. We use the formula SE(Median)= 1.25*SE(Mean)
...
## The order must be mainntained as it is
...
fig5 <- ggplot(df5, aes(x = ATET, y = y_position, color = Y)) +
  ...
  labs(y = "Tree growing outcomes") +
  scale_x_continuous(labels = function(x) paste0(x, "%")) +  # Just add "%" to x-axis labels without conversion
...
fig5
## % sign on numbers on horizontal axis of fig5
## Removed ATET from X axis of Fig 5
...
agftreat<- read_dta("AGF treatment by village.dta")
names(agftreat)
...
## Importing a temporary data set (see: "Commercial tree groing by farmers in MP, India.do" )
treeage <- read_dta("TemporaryDataset.dta")
names(treeage)
...
## A subset of covariates from TemporaryDataset is imported (see: "Commercial tree groing by farmers in MP, India.do" )
## This subset drops the observations who recived Treaament 2 (Participated in Program)
matching<- read_dta("Vars for COVSEL_Treat level 1.dta")
...
## A subset of covariates from TemporaryDataset is imported (see: "Commercial tree groing by farmers in MP, India.do" )
## This subset drops the observations who recived Treaament 1 (Knew of Program)

matching<- read_dta("Vars for COVSEL_Treat level 2.dta")
